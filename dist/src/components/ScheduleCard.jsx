...
function ScheduleCard() {
    ...
    const getNextOccurrence = (dayOfWeek, timeString) => {
        const today = new Date();
    ...
    return (<div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          <lucide_react_1.Calendar className="w-6 h-6 text-slate-600"/>
          <div>
        ...
        <button onClick={exportAllToGoogleCalendar} className="flex items-center space-x-3 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-md hover:shadow-md transition-all duration-200 text-sm font-medium shadow-sm hover:bg-gray-50">
      ...
      <div className="grid grid-cols-5 gap-3">
        {weekDays.map((day, index) => {
            ...
            return (<div key={day} className="min-h-[140px]">
              {/* Day header */}
              <div className={`text-center p-3 rounded-t-lg border-b ${isCurrentDay
                    ? 'bg-blue-50 border-blue-200 text-blue-900'
                    : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                <div className={`text-sm font-bold ${isCurrentDay ? 'text-blue-600' : 'text-slate-900'}`}>
                  {day}
                </div>
              </div>
              ...
              <div className="p-3 border-l border-r border-b border-slate-200 rounded-b-lg bg-white min-h-[100px]">
                <div className="space-y-2">
                  {daySchedule.map((item, idx) => (<div key={idx} className={`p-3 rounded-md border text-xs ${getTypeColor(item.tipo)} relative group`}>
                      {/* Export button for individual class - Google style */}
                      <button onClick={() => exportToGoogleCalendar(item)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-white/70 rounded-full bg-white/50 backdrop-blur-sm" title="Exportar para Google Calendar">
                      ...
                      {item.sala && (<div className="flex items-center space-x-1">
                          <lucide_react_1.MapPin className="w-3 h-3"/>
                          <span>{item.sala}</span>
                        </div>)}
                    </div>))}
                  {daySchedule.length === 0 && (<div className="text-center py-4 text-slate-400 text-xs">
                      Sem aulas
                    </div>)}
                </div>
              </div>
